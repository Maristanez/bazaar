...
**Redesigned Sat 19 Sep 20:00 (grilled, Bryan).** The Console answers one owner question — *what does the shopkeeper earn me that a list-price store would lose?* — and lets her try a setting on simulated shoppers before real ones meet it. One viewport at 1360 px, no tabs; below 760 px it stacks KPIs → settings list → setting in focus → feed. **Layout chosen Sat 21:00 after three prototype layouts** (`mockups/console-redesign.html`, `?variant=A|B|C`): showing every control at once read as busy, so the page shows one setting at a time; the bar lanes from variant B and the takings receipt from variant C were kept. `DESIGN.md` tokens are unchanged.
...
| **KPI strip — real deals** | One band from the `deals` table led by **You kept** (profit recovered − agent cost) and profit per $1 of agent cost, labelled `on N real deals`: **Customers saved** · **Revenue recovered** · **Profit recovered** · **Agent cost**. Secondary line: `vs 20% banner`. Follows the product picker. A `settled` event updates the tiles live. No deals ⇒ dashes and "No deals yet"; never sample data. |
| **Left — settings list** | Every owner setting as one line with its saved value (`Floor 25%` · `Max off list 22%` · `Rounds 4` · `Voice Friendly` · `Lowball 40%` · `Rate limit 30/h` · `Blocked 1` · `Products 9`), a changed marker on any line whose candidate value differs, and one **Adopt** for the whole candidate policy. The Console stays complete — everything is controllable — but only one setting is open at a time. |
| **Centre — the setting in focus** | Clicking a line opens that setting alone: its control at full size, one sentence on what it does, and **"If you adopt this"** — the Forecast for the candidate policy: three store lanes (**no-agent store**, **banner**, **shopkeeper**) as bars of the 300 simulated shoppers with profit at the end of each, then the figures this setting moves as **saved → candidate** with the delta. The dot histogram (§9.2) sits under the lanes. Product picker (every product in the mirror; "All products" forecasts the best-seller). Re-runs on every change; nothing reaches the storefront until Adopt. Voice, rate limit and blocklist say "does not change prices" in place of a delta. *Products* in focus lists the firm-price switches, which rule binds (`floor` / `cap`), **missing cost → not open to offers** (coral), **missing `stocked_at` → treated as new stock** (amber); *Guardrails* carries the red-team badge (`20 attacks · 0 breaches`). |
| **Bottom — needs you** | The yellow **Approve / Decline** card: items, shopper's offer, **profit in $ and as "% over cost"**, 45-second bar |
| **Bottom — live feed** | The three newest rows, expandable to the full feed. Compact rows, click to expand the code-composed reasoning (offer, floor, menu, pick, memory, model, ms, `cost_usd`), surface tag, **Block** button. **Blocked rows in coral**, naming the layer: `filter · validate · engine · check · auditor`. A lowball counter is a neutral row (`Lowball · countered at $133 · no LLM call`) — nothing was refused. |
...
All on the **rounded-up whole-dollar** figures unless a clause says otherwise: no option, offer, or minted deal is ever ≤ cost · no `stocked_at` ⇒ urgency 0 · none below floor unless owner-approved · no **held, final or something-else** option below `target` of its cart unless owner-approved; a **bundle** is bound differently — its shoe part ≥ `target(main product)` and its total ≥ `floor(cart)` (the add-on's discount is the trade, so a new-stock bundle sits below its cart's list = target by design) · rounded `ask(r)` never increases and `ask(4) = target`; **step shrinkage is asserted on the unrounded cents** (rounding up to whole dollars can make a later step $1 larger) · never counters below the shopper's offer **on the product they offered on** (option A and its bundles); a something-else option is a different product and may sit below the offer, bound by its own cart's target · policies are generated with `floor(cart) ≤ list(cart)`; a floor above list yields **no option** for that cart — dropped, never clamped · an accepted offer that is not a whole dollar rounds **up** · urgency 0 ⇒ target = list ⇒ price never moves · urgency 1 ⇒ target = floor · a bundle cart uses its main product's urgency · add-on part is exactly `cost + ½ margin` · "something else" fires only when `x < target(p)` or `r ≥ 3`, only returns `target(q) ≤ max(x, budget)`, and prices at `max(target, min(ask, budget))` · after an owner decline the restated offer equals the final offer · accept rejects unknown / expired / used / superseded offer ids · missing cost ⇒ no menu.
