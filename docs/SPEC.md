...
5. **Reasons must be true.** A reason in the shopkeeper's line must be one of the picked option's `facts`; anything else fails the check. The engine writes each option's `facts` (`factsFor` in `packages/engine/src/negotiation-menu.ts`), and they are public-safe by construction: **`"<add-on title> included"`** for a bundle; **the shopper's own reason handed back** as a fixed string per reason label (`"to fit your budget"`, `"for buying more than one item"`, `"for a returning customer"`, `"for what you have planned"`, `"since you're ready to check out today"`), and only on an option priced below list, so a reason is never credited where it bought nothing; and **`"meets your $N budget"`**, where `$N` is the shopper's own offer and the option's total is at or under it. **Stock age never becomes a fact** — it is leverage against the owner — and neither does a price-match claim, because no competitor price was verified. A fast-check property holds the line: the only dollar figure any fact carries is the shopper's own. With no facts on an option, the LLM's line is the neutral offer sentence alone. Code, never the model, may answer the reason the shopper gave in that message: the engine's `reasonReply` (*"A tight budget. I've been there."*) opens the code fallback line, and the server puts it in front of the model's checked sentence; on the last round the server adds the engine's closing words after the model's sentence (*"That's the last stop on this trail."*); the code fallback line carries its own ending, because it holds the price itself (*"Last stop on this trail: $141. I can't go past it."*). The template line used when the floor changes mid-turn is code-written and goes out as it is. Both are fixed strings with no figure in them — the shopper's own reason handed back, never a claim about the shop. No invented scarcity. The 15 minutes is real because the code really expires.
...
- **§4.2.1 The immersive shopkeeper** (decided 20 Sep; tasks V1–V13 in `PLAN.md`, findings in `research/immersive-voice-widget.md`):
  - **Owner decisions of 20 Sep, overriding the first draft of this section.** The shopkeeper is named **Jarvis** (the chat header, every on-page line and the server persona in `packages/llm/src/backboard.ts`); theme file names keep the `juniper-` prefix. **"Hey Jarvis" is always armed** on a browser that can hear (no toggle; `BazaarChatFlags.keyword: false` is the kill switch) and **does not open the chat**: it starts hands-free with the chat closed, the voice pill shows he is listening, and what is said appears in a three-line bubble above him; a tap opens the chat. `BazaarChatFlags.autoListen: 'always'` listens from page load. Manual push-to-talk is retired for now; the small mic is the hands-free toggle. **Hearing has two engines** (`juniper-ears.js`): Chrome's `SpeechRecognition` in Google Chrome, and elsewhere (or when Chrome's fails) a recorder with pause detection that posts to `/api/voice/transcribe`. The listening state is always visible (pill, header bars, coral mic).
  - **The chat's shape (V12) and Jarvis's hands (V13)** are as recorded in `PLAN.md`. Trust boundary for V13: a turn that is only a chore never reaches the server (the server reads every turn as bargaining); he never presses Deal, checks out, pays, or writes a price (invariant 1), and cart edits happen only on the shopper's words, each with Undo.
  - **Hands-free is opt-in and always visible.** One click starts it; it lasts the session and survives page loads. Closing the panel leaves a listening pill rather than a hidden microphone. *(Superseded above: the keyword is always armed and the shopkeeper is Jarvis.)*
  - **Hearing:** Chrome `SpeechRecognition` — live caption, turn ends after ~1.2 s with no new result, the text goes through the same turn as a typed message. The shopper is told once that Chrome's speech service receives the audio. Where it is missing, push-to-talk through `/api/voice/transcribe` stays as it is.
  - **Speaking:** still `/api/voice/speak`. Half-duplex — the recogniser is paused while she speaks. **No hosted speech-to-speech agent** (OpenAI Realtime, ElevenLabs Agents, Hume): the model would say prices itself, past the menu and the check (invariant 1). **No in-browser wake-word SDK**: it puts a key in the bundle (invariant 6).
  - **Page awareness:** the theme sends `page` (type, product, collection, search terms, cart) on `POST /api/chat` and `POST /api/greeting`. The spoken page-aware opening is recalled by the server; the one silent launcher nudge is a fixed label with no figure.
  - **A clean visit** is a typed or pasted `?shopper=` URL. A page reached from a link inside the chat, or with hands-free on, carries the conversation.
  - **Chips** are built by widget code from page, cart, selection, card state and reasons already used, and may be replaced by server `suggestions` (code-written from what the public card shows, through the wording check). Still never a dollar figure.
  - **Motion** follows §12: the panel unfolds like the paper map, the sticker travels launcher → header, chips and cards land with the same slap; all of it is instant under `prefers-reduced-motion`.
...
**Railway is only half the deploy.** The shopkeeper agent is not deployed from this repo: the base assistant, its indexed store documents and every shopper's memory live on **Backboard**, provisioned in Backboard's dashboard (§8). A deploy ships the client, a key and an assistant id — it never creates an assistant, never uploads a document, and never checks that one was indexed. The per-shopper assistant clones live there too, so unlike in-flight haggles they survive a restart. Changing `BACKBOARD_MODEL_NAME` in the host's settings swaps the model with no code change, and silently overrides the default in the server.
...
**Where the agent lives.** On Backboard, not on our host. The base assistant (`BACKBOARD_ASSISTANT_ID`) is created once by hand in the Backboard dashboard; the three store documents in `infra/seed/` are uploaded to it there and must reach `indexed` before they answer anything; each identified shopper gets a clone of it, created at runtime, holding that shopper's memory. None of that is in this repo and none of it is recreated by a deploy. Moving to a new host costs only the environment variables — the assistant is still on Backboard; it is a fresh Backboard account, or a wiped or replaced assistant, that costs the dashboard steps again (`mattpocock-skills:wizard`), including re-uploading the documents and waiting for `indexed`. What our deploy carries is the client, the key, the assistant id and the model name.
...
| **Memory** | Size, what they're training for, what they liked; seeded for the `demo` shopper; mode from `BACKBOARD_MEMORY_MODE`; `memory_response_citation` on for owner telemetry; raw citation markers stripped from shopper prose | The opening line of a `?shopper=` visit, which is whatever `POST /api/greeting` recalled (*"Welcome back — still a size 10?"* for the seeded demo shopper) and is absent when nothing was; feed shows the recalled memory |
...
Whether `minimumRequirement.subtotal` voids the code when a bundle item is removed at a real checkout, and whether the amount spreads across the cart there (the code sets `appliesOnEachItem: false`; the red team runs with dry-run discounts, `liveShopifyValidated: false`) · whether draft-order invoice links open behind the store password (the backup settlement is not built) · whether Railway keeps an SSE connection open past 60 s (we send a comment every 15 s) and really runs one instance · the routed model's latency against the 6500 ms timeout · Backboard rate limits · **tone preset and firm-price ids are stored in `PolicySettings` but nothing reads them yet** · for the planned ChatGPT surface: which card MIME type / meta key a ChatGPT build needs, and whether `_meta["openai/subject"]` is present in developer mode · font licences · vendor-claimed competitor stats (don't cite them) · for hands-free voice (§4.2.1, `research/immersive-voice-widget.md`): how long Chrome's `SpeechRecognition` runs before it stops itself and whether it restarts without a gesture after a page load, whether the ~1.2 s end-of-turn timer suits a noisy hall, whether echo cancellation covers the page's own `<audio>` (we pause the recogniser while Jarvis speaks instead), and how reliably Chrome hears "Jarvis" on the demo laptop. Never say SimGym "excludes" pricing — only that it is about themes.
