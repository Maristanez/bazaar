...
- **§4.2.1 The immersive shopkeeper** (decided 20 Sep; tasks V1–V11 in `PLAN.md`, findings in `research/immersive-voice-widget.md`):
  - **Hands-free is opt-in and always visible.** One click starts it; it lasts the session and survives page loads. Closing the panel leaves a listening pill rather than a hidden microphone. Nothing listens before that click (rule 10). The optional keyword is **"Jarvis"**: it only opens hands-free; the shopkeeper is still Juniper.
