...
**Redesigned Sat 19 Sep 20:00 (grilled, Bryan).** The Console answers one owner question — *what does the shopkeeper earn me that a list-price store would lose?* — and lets her try a setting on simulated shoppers before real ones meet it. One viewport at 1360 px, no tabs; below 760 px it stacks KPIs → settings list → setting in focus → feed. **Simplified Sat 22:30 after the race prototype (`mockups/race-prototype.html`): three things on the first screen — real money kept, try a setting on 300 shoppers, live approvals — and everything else behind More settings.** Earlier: layout chosen Sat 21:00 after three prototype layouts (`mockups/console-redesign.html`, `?variant=A|B|C`): showing every control at once read as busy, so the page shows one setting at a time; the bar lanes from variant B and the takings receipt from variant C were kept. `DESIGN.md` tokens are unchanged.
