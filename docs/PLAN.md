...
- **Deploy config:** `railway.json` builds the Console (`pnpm build`), starts with `pnpm start`, health check `/health`.
...
| LLM latency makes the haggle feel dead | H | Thinking face at once; 6.5 s cap (`BACKBOARD_TIMEOUT_MS`) → option A + template; warm threads. Observed offer runs 0.9–2.3 s locally on the earlier default model; **not re-measured on `gpt-5.6-terra`** (R11, E3) |
