...
- [ ] **CR2** New layout + Forecast: settings list left, one setting in focus centre (store lanes, saved → candidate deltas, B12 histogram, product picker), approvals + compact feed below; discount cap in the engine [§4.4, §4.4.1] · _needs CR0, CR1, B11_ · ~1.25 h · **Done when:** dragging floor or cap changes the candidate column and leaves the saved column and the live policy untouched; Adopt makes them equal; property: every option ≥ `max(floor, list × (1 − cap))`; at cap 22 every existing engine test still passes — **closes B12**
