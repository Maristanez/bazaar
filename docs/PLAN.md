...
- [ ] ~~**S9** Click-a-dot mini-transcript + persona legend as a filter [§9.2] · _needs B12_ · ~1.5 h · **Done when:** any dot shows persona, willingness, offers and asks per round, the trade, the outcome~~ — cut Sat 22:30: folded into CR2 — pointing at a race dot gives the shopper's story
...
- [ ] ~~**CR6** Rate limit + blocklist + message cap, `blockedBy: "filter"` [§4.4.1, §7] · _needs CR3_ · ~0.75 h · **Done when:** the 31st message in a minute from one address, a blocked address, and a 1,001-character message each produce a coral `filter` row and no LLM call; changing `shopperId` does not reset the count; Unblock restores offers~~ — cut Sat 22:30: pays for the animated price race; `shopperId` is client-supplied so a real limiter needs address keying we cannot verify on venue Wi-Fi. The lowball counter remains the anti-abuse feature
