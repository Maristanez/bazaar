...
- [ ] **CR10** The opening greeting for the `demo` shopper comes from recalled memory, or `SPEC.md` §8 stops citing it as the memory proof (`apps/storefront/assets/chat-demo.js:91` hardcodes it today) · ~0.75 h
...
| Two people using the same shopper id share one Backboard thread | M | `threadKey` is `[shopperId, negotiationId]` (`packages/llm/src/backboard.ts:524`) and a catalog question defaults `negotiationId` to `catalog:<shopperId>` (`apps/server/src/application.js:322`), so every caller using the same id shares one transcript. Real shoppers are safe — the widget mints a random id per browser — but **`?shopper=demo` is a shared id**, so demo runs are not clean starts and one judge could see the previous one's conversation. The in-process thread map is cleared by a restart, so restart the host before judging, then warm exactly the thread you will demo |
