...
- [x] **V1** Hands-free conversation: one click on "Talk to Juniper" starts Chrome `SpeechRecognition`; the shopper's words show as a live caption; ~1.2 s with no new result ends the turn and sends it as a normal chat turn; when Juniper's reply audio ends the mic reopens. Recognition is paused while she speaks (half-duplex). Click, Esc and the Stop button interrupt her; a spoken "stop" interrupts only behind a flag. One line tells the shopper the audio goes to Chrome's speech service. Without `SpeechRecognition`, today's push-to-talk is unchanged · ~1.5 h · **Done when:** in the harness, a scripted two-turn haggle completes with no click after the first, and with `SpeechRecognition` removed from `window` the mic button still records and sends
...
- [x] **V3** Open / close motion and the voice pill: the panel unfolds like the paper map from the launcher's corner and the launcher sticker travels into the header (~280 ms open, ~200 ms close, `@starting-style` + `allow-discrete`); closing with hands-free on leaves a pill — Juniper's head, mic halo, latest caption — that keeps listening. Instant under `prefers-reduced-motion` · _needs V1_ · ~1 h · **Done when:** open, close and pill each animate in Chrome, and with reduced motion on all three switch with no animation
- [x] **V4** Live presence: one `AnalyserNode` on the mic drives the halo and the wave bars; one on the reply audio moves Juniper's mouth with the volume · _needs V1_ · ~0.75 h · **Done when:** the bars sit flat in silence and rise with speech, and the mouth is still when the audio is paused
...
- [x] **V7** Juniper points at the page: when a reply names a product that is on the current page, the page scrolls to it and rings it for two seconds · ~0.75 h · **Done when:** on the collection page a suggested product scrolls into view with the ring, and on a page without it nothing moves
...
- [x] **V10** Two earcons (listening starts, reply ready), `role="log"` on the messages and a `role="status"` line for the voice state · ~0.25 h · **Done when:** a screen reader announces "Listening" and "Juniper is speaking"
