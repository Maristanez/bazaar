...
**Console redesign (Sat 20:00 → Sun 00:30, hard cap 4 h, grilled Sat 19 Sep — SPEC §4.4, §4.4.1, §4.4.2).** Ordered keep-first: the Console is demoable after CR3; CR6 is dropped first. Every contract change is additive and optional (`PolicySettings`, `DealKpis`, `CatalogStatus`) — **tell Ritvik and Ricardo before merging CR1.** Prototype the layout in `mockups/console-redesign.html` (≤ 45 min, `impeccable`) and get Bryan's yes before CR2.

- [ ] **CR0** Static HTML prototype of the §4.4 layout on the `DESIGN.md` tokens · ~0.75 h · **Done when:** Bryan approves the layout at 1360 px and 390 px
...
- [ ] **CR2** New layout + Forecast: controls left, Forecast centre (three stores side by side, saved → candidate deltas, B12 histogram, product picker), approvals + compact feed right; discount cap in the engine [§4.4, §4.4.1] · _needs CR0, CR1, B11_ · ~1.25 h · **Done when:** dragging floor or cap changes the candidate column and leaves the saved column and the live policy untouched; Adopt makes them equal; property: every option ≥ `max(floor, list × (1 − cap))`; at cap 22 every existing engine test still passes — **closes B12**
- [ ] **CR3** Lowball counter + owner-set max rounds + Guardrails group with red-team badge [§4.4.1] · _needs CR2_ · ~0.75 h · **Done when:** an offer under the cutoff returns a public card with a reason fact, makes no LLM call, leaves `round` unchanged, and forty in a row return the same price; an offer under every option's total returns a *something else* card; max rounds 2 and 6 each end on `ask(N) = target`, and at 4 the B1 worked example is unchanged; the Forecast counts a lowballer as saved or walked by willingness
...
- [ ] **CR6** Rate limit + blocklist + message cap, `blockedBy: "filter"` [§4.4.1, §7] · _needs CR3_ · ~0.75 h · **Done when:** the 31st message in a minute from one address, a blocked address, and a 1,001-character message each produce a coral `filter` row and no LLM call; changing `shopperId` does not reset the count; Unblock restores offers
- [ ] **CR7** `settings jsonb` migration + in-memory fallback note · _needs CR2_ · ~0.25 h · **Done when:** with the column absent, Adopt succeeds, the Console shows the retry note, and a restart test proves settings reload when the column exists
