...
**Last updated:** Sat 19 Sep 20:27 EDT. Backboard now defaults to OpenAI `gpt-5.6-terra`. Quantity price questions no longer become tiny offers, explicit per product prices become one cart total, and LLM model questions get a direct truthful answer. All 287 tests, workspace type checks and the production build pass. Hosted deployment acceptance remains open.
...
| ⚙️ B. Engine, core, Gym + Console | Bryan | **Console redesign (CR0–CR7, SPEC §4.4)** decided Sat 19:45 after grilling: real-deal KPIs vs a no-agent store, Forecast (saved → candidate), new owner settings, filter layer. S8 / S10 cut to pay for it. Branch `console-redesign`; decisions written, no code yet. Earlier work (owner routes, approvals, PAUSE, Gym, isolated red-team evidence, explicit multi-item cart preservation) stays verified locally | CR0 prototype → CR1 KPIs → CR2 layout + Forecast → CR3 lowball + rounds (demoable) → CR4 tone → CR5 firm price → CR6 rate limit. Then: apply migrations, publish theme, verify Railway | Supabase keys missing from this machine's root `.env` (Console cannot sign in; Shopify mirror verified live, 9 products) · `impeccable` skill not installed · Shopify / Railway / Supabase dashboard access |
...
| 🤖 D. Agents + guardrails | **Ricardo / Codex for Backboard; remaining guardrails need an owner** | Shopper Q&A and checked offer wording route through Backboard to OpenAI `gpt-5.6-terra`. Each identified shopper gets an isolated cloned Assistant with indexed documents and writable memory. The complete server-priced menu reaches Backboard, and explicit bundles cannot be silently reduced to one item. | Finish the proactive demo-shopper greeting for R10, then R9 *understand* and hosted R11 verification | Ricardo / Codex for R10/R11 |
...
| ⚙️ B. Engine, core, Gym + Console | Bryan | **9 / 20** | `████░░░░` | B1 · B2 · B3 · B4 · B5 · B6 · B11 · S5 · S6 | CR0 · CR1 · CR2 (closes B12) · CR3 · CR4 · CR5 · CR6 · CR7 · S9 · S7 · ~~S8~~ · ~~S10~~ |
...
- [x] **B12** Functional static dot histogram + metric cards [§9.2] · _needs B11, S6_ · ~2 h · **Done when:** one dot per shopper coloured by persona, grey outline for policy A, reference lines, shaded cost → floor band, and the headline card turns red when haggling loses
- [ ] ~~**S9** Click-a-dot mini-transcript + persona legend as a filter [§9.2] · _needs B12_ · ~1.5 h · **Done when:** any dot shows persona, willingness, offers and asks per round, the trade, the outcome~~ — cut Sat 21:15: folded into CR2 — pointing at a race dot gives the shopper's story
...
- [ ] ~~**CR6** Rate limit + blocklist + message cap, `blockedBy: "filter"` [§4.4.1, §7] · _needs CR3_ · ~0.75 h · **Done when:** the 31st message in a minute from one address, a blocked address, and a 1,001-character message each produce a coral `filter` row and no LLM call; changing `shopperId` does not reset the count; Unblock restores offers~~ — cut Sat 21:15: pays for the animated price race; `shopperId` is client-supplied so a real limiter needs address keying we cannot verify on venue Wi-Fi. The lowball counter remains the anti-abuse feature
...
- [ ] **R11** Backboard choose + say: an OpenAI model routed through Backboard, `OPTION: X` + line, buffered, 4 s timeout, `run_failed` handling, `cost_usd` [§8] · _needs R10, B2_ · ~2 h · **Done when:** **latency is measured and written into §9 of this file**, and the fallback fires on a forced timeout
  _Sat 13:57 Codex note:_ the Gemini shopper answer path now routes through Backboard to OpenAI `gpt-4o`. The REST streaming adapter buffers through `run_ended`, handles `run_failed` and `error`, records model, latency, `cost_usd`, memory and thread metadata, reuses a thread per shopper plus negotiation, and times out safely. A live local server run answered the sizing question from the indexed knowledge and recalled the seeded size 10 memory, then reused the same thread for choose plus say. The checked choose plus say run took **1,789 ms** and cost **$0.006935**. The tracker stays open because the configured demo still uses one shared Assistant rather than creating one Assistant per shopper, and the current JavaScript offer route gives Backboard one code approved option while the typed B5 port is ready for the complete B2 menu.
  _Sat 14:02 model selection note:_ live comparison selected OpenAI `gpt-4.1-mini` as the default balance. It retrieved the seeded memory and all three indexed documents, answered the sizing question in **3,906 ms** for **$0.0009376**, then produced a check passing offer in **1,401 ms** for **$0.0006112**. `gpt-4o-mini` was cheaper but added unsupported wording to the offer, while `gpt-5-mini` exceeded the ten second comparison timeout.
...
| LLM latency makes the haggle feel dead | H | Thinking face at once; 4 s cap → option A + template; warm threads. **Measured choose + say latency: 1,401 ms through Backboard to OpenAI `gpt-4.1-mini`** |
