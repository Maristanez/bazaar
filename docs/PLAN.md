...
| ⚙️ B. Engine, Gym + Console | Bryan | **Sun 05:50 — the immersive shopkeeper (V1–V13) is built, merged to `main` and on the LIVE theme** (rollback copy: unpublished theme `live-backup-before-jarvis`, #161277542597): Jarvis hears on any browser, "Hey Jarvis" without opening the chat, hands-free turns, page awareness, changing chips, a movable / resizable / minimisable chat, and autonomous chores on the page (products, sizes, quantities, cart edits, navigation) — 216 storefront tests, full suite and typecheck green. What he is and every file: `apps/storefront/README.md`. Earlier: **Repo reorganised Sat 22:45** on branch `cleanup/one-engine-apps-central`: one engine behind `@bazaar/engine`, dead modules deleted, theme at `apps/storefront/`; 262 tests and typecheck green. Console redesign CR0–CR3 and CR7 are built: real-deal KPIs, the price race with Floor / Max off / Rounds / Lowball pills, Adopt, the live rail. S8 / S10 were cut to pay for it. **Sun 00:50:** the negotiation UX plan ([`negotiation-ux-plan.md`](negotiation-ux-plan.md), tasks 4–8: Juniper, look A, shopper-language card, chips, Make an offer button, accessibility) is built — theme live, engine + server half uncommitted in the working tree, 301 tests green | **Deploy the server from `main` on Railway** (pushing did not trigger it; production still answers with the old persona, without the refusal rules or the page-aware greeting — the theme covers the name and the listed refusals meanwhile). Then: **Commit and push the negotiation UX server half (redeploys Railway — before the 08:00 freeze), then edit the eight product descriptions in Shopify admin.** CR4 tone → CR5 firm price. Then: apply `infra/migrations/20260920_policy_settings.sql` (until then owner settings are lost on restart), publish the theme, verify Railway | Supabase keys missing from this machine's root `.env` (Console cannot sign in; Shopify mirror verified live, 9 products) · Shopify / Railway / Supabase dashboard access · Railway CLI not installed or logged in on this machine, so the server deploy needs the dashboard or a teammate |
...
### Live QA handoff — Sun 20 Sep 01:45 EDT · Bryan / Codex

- **Now:** Implemented the report's cart/size/amount, held-price, routing, and shopper-copy fixes. Preserved the existing memory isolation work and verified a fresh identity against real Backboard. Added 25 transcript API regressions plus catalog, theme and engine cases. Full-suite checkpoint: 371 tests / 42 files pass; typecheck and build pass. See [`live-chat-stress-test-2026-09-20.md`](live-chat-stress-test-2026-09-20.md) for the implementation matrix and original live evidence.
- **Next:** Publish the tested server/theme, apply the corrected Shopify product descriptions and Checkout branding, then replay the hosted regression and Console checklist. The authenticated follow-up confirmed automatic feed delivery, but production still treats 20% as $20 and typed size 10 as size 9. Trail Cap currently has no cost and is excluded: correct merchant data or rehearse an alternative before the bundle pitch.
- **Blocked:** Railway CLI is not authenticated, so no deployment was submitted. Owner sign-in worked briefly; its tab then became unavailable, and the reopened Console needs sign-in again. Approve/Decline, PAUSE/resume, model traces, store copy/branding and shipping/tax totals remain unverified. Other work is concurrently advancing this checkout; preserve its changes when staging/publishing.
...
| ⚙️ B. Engine, Gym + Console | Bryan | **Sun 05:50 — the immersive shopkeeper (V1–V13) is built, merged to `main` and on the LIVE theme** (rollback copy: unpublished theme `live-backup-before-jarvis`, #161277542597): Jarvis hears on any browser, "Hey Jarvis" without opening the chat, hands-free turns, page awareness, changing chips, a movable / resizable / minimisable chat, and autonomous chores on the page (products, sizes, quantities, cart edits, navigation) — 216 storefront tests, full suite and typecheck green. What he is and every file: `apps/storefront/README.md`. Earlier: **16 / 18** | **Deploy the server from `main` on Railway** (pushing did not trigger it; production still answers with the old persona, without the refusal rules or the page-aware greeting — the theme covers the name and the listed refusals meanwhile). Then: B1 · B2 · B3 · B4 · B5 · B6 · B11 · B12 · S5 · S6 · S7 · CR0 · CR1 · CR2 · CR3 · CR7 | CR4 tone · CR5 firm price (S8 · S10 cut) · Railway CLI not installed or logged in on this machine, so the server deploy needs the dashboard or a teammate |
...
**Immersive shopkeeper (Sun 03:30 →) — build in this order, cut from the bottom** [SPEC §4.2.1 · findings in `research/immersive-voice-widget.md`]. Theme-only unless marked *server*; the theme sends the new fields either way and the server ignores what it does not read, so no task waits on a deploy. Feedback loop for every task: `scripts/chat-walkthrough/harness.mjs` and `node scripts/storefront-preview.mjs`; push with `--unpublished` and check in Chrome before `--allow-live`. UI tasks (V3, V4, V6, V10) go through `impeccable` against SPEC §12.

- [x] **V1** Hands-free conversation: one click on "Talk to Jarvis" starts Chrome `SpeechRecognition`; the shopper's words show as a live caption; ~1.2 s with no new result ends the turn and sends it as a normal chat turn; when Jarvis's reply audio ends the mic reopens. Recognition is paused while he speaks (half-duplex). Click, Esc and the Stop button interrupt him; a spoken "stop" interrupts only behind a flag. One line tells the shopper the audio goes to Chrome's speech service. Without `SpeechRecognition`, today's push-to-talk is unchanged · ~1.5 h · **Done when:** in the harness, a scripted two-turn haggle completes with no click after the first, and with `SpeechRecognition` removed from `window` the mic button still records and sends
- [x] **V2** The conversation survives navigation: transcript, card id, panel state and the hands-free flag live in `sessionStorage`; a page load reached from a link inside the chat, or with hands-free on, skips `/api/session/reset` and restores the chat, re-reading the card from `GET /api/offers/:id`. A typed or pasted `?shopper=` URL is still a clean visit · ~0.75 h · **Done when:** following a product card keeps the transcript and the live card, and pasting the `?shopper=demo` URL in a new tab starts empty
- [x] **V3** Open / close motion and the voice pill: the panel unfolds like the paper map from the launcher's corner and the launcher sticker travels into the header (~280 ms open, ~200 ms close, `@starting-style` + `allow-discrete`); closing with hands-free on leaves a pill — Jarvis's head, mic halo, latest caption — that keeps listening. Instant under `prefers-reduced-motion` · _needs V1_ · ~1 h · **Done when:** open, close and pill each animate in Chrome, and with reduced motion on all three switch with no animation
- [x] **V4** Live presence: one `AnalyserNode` on the mic drives the halo and the wave bars; one on the reply audio moves Jarvis's mouth with the volume · _needs V1_ · ~0.75 h · **Done when:** the bars sit flat in silence and rise with speech, and the mouth is still when the audio is paused
- [x] **V5** Page awareness: Liquid emits a `data-ai-chat-page` JSON blob (page type, product, collection, search terms); the widget adds `/cart.js` when the panel opens and sends it all as `page` on `POST /api/chat` and `POST /api/greeting`. *Server:* both routes accept `page`; with hands-free on, the greeting for a new page is recalled with that page in context — never a string in the theme · ~1 h · **Done when:** on the collection page with socks in the cart, the `page` field in the request names both, and a request with no `page` still returns today's greeting
- [x] **V6** Suggestion chips that change: code in the widget builds them from the page, the cart, the picked variant and quantity, the card's state and the reasons already used; a used chip does not come back; no chip carries a dollar figure. New chips land one by one with the card "slap", a tapped chip becomes the shopper's bubble, and in hands-free the row reads "Try saying…" and the chip being spoken lights up. *Server, layered on top:* `suggestions` on the chat reply, written by code from the trade types and product names the public card would show, passed through the wording check · _needs V5_ · ~1.25 h · **Done when:** the same product page shows different chips at quantity 1, at quantity 2 and after a final offer, and a test proves no chip string contains a digit after `$`
- [x] **V7** Jarvis points at the page: when a reply names a product that is on the current page, the page scrolls to it and rings it for two seconds · ~0.75 h · **Done when:** on the collection page a suggested product scrolls into view with the ring, and on a page without it nothing moves
- [x] **V8** The "Jarvis" keyword: a first click on "Turn on Hey Jarvis" is remembered in `localStorage`; while the chat is closed a recogniser listens for the word only, the launcher shows a listening ear, and hearing it opens hands-free. Off by a theme setting if it misfires in the hall · _needs V1, V3_ · ~0.75 h · **Done when:** saying "Jarvis" with the chat closed opens hands-free, the ear is visible whenever the recogniser runs, and with the setting off no recogniser starts
- [x] **V9** One silent nudge per session: after a dwell on a product page the launcher label changes once ("Too steep? Name a price."), no audio · ~0.5 h · **Done when:** it shows once, never again in that session, and never while the chat is open
- [x] **V10** Two earcons (listening starts, reply ready), `role="log"` on the messages and a `role="status"` line for the voice state · ~0.25 h · **Done when:** a screen reader announces "Listening" and "Jarvis is speaking"
- [x] **V11** *(server, last)* Spoken filler while the shopkeeper picks: two or three cached clips, played only if the reply takes longer than 1.5 s · ~0.5 h · **Done when:** a fast reply plays no filler and a slow one plays exactly one
- [x] **V12** The chat keeps its shape out of the conversation's way: smaller by default (376 × 576), resizable from its top-left corner up to 560 × 860, movable by its header, one minimise button (the conversation is kept), suggestions on a single scrolling line. Jarvis reshapes his own panel for an offer card and while listening, until the shopper picks a size. At rest he is only his face, which can be carried anywhere and goes home to the bottom-right corner on minimise. Minimised, what is said moves through a three-line bubble above him: the shopper's newest words, then his answer at the pace of his voice · **Done when:** `tests/storefront/juniper-shape.test.ts` passes
- [x] **V13** Jarvis's hands: he does the shopping chores he is asked for, on the page, in plain sight — opens products, picks sizes, sets quantities, adds to the cart, removes an item, empties the cart (each with Undo), opens the cart, goes home / back / forward, refreshes, scrolls. A chore for another page takes him there and is finished on arrival; asked to find something, he takes the shopper to the product he named ("Stay here" stops him). A turn that is only a chore is answered on the page (`BazaarChat.answerLocally`) and never reaches the server. He never presses Deal, checks out, pays or writes a price · **Done when:** `tests/storefront/juniper-hands.test.ts` passes
