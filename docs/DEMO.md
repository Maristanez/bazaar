...
> **Current verification note (2026-09-19).** The live store is [b8wzw0-h3.myshopify.com](https://b8wzw0-h3.myshopify.com/), while the Railway server/theme changes remain unpublished and `/console` is currently unavailable. The live homepage shows eight products and omits Trail Runner 3; the local theme fix has not been published. Local storefront tests reached the real Shopify, Backboard and Supabase services, but do not prove the published theme. The Gym is the price race: 300 rule-based shoppers priced by the live engine, with no LLM in it; the current Trail Runner 2 seed-42 floor sweep remains negative versus the 20% banner, so the demo must not promise that a slider turns that figure positive.
...
- **OpenAI:** "An OpenAI model, routed through Backboard, turns a shopper's sentence into a structured offer and picks the deal from the menu — and Codex wrote our property tests and found a real bug — here's the log."
...
**Open on the haggle within ten seconds — never on architecture.** A chat window alone reads as a wrapper; the three things that must fill the screen are the **real checkout**, the **owner's Approve card**, and the **price race settling**.
...
> **The ChatGPT beat (beat 7) is planned and not built on main as of Sat 19 Sep evening** — there is no `/mcp` route and no widget. If it is not built by the gate, the demo ends on the beat before it: beat 6, then the closing line.
...
5. **(2:05) The Gym.** "She'd seen a deterministic rehearsal before it met a customer. The Gym uses the live `priceOffer` engine with 300 synthetic, rule-based shoppers." Press **▶ Play** on the price race: the dots wait at what each shopper would pay, the ask line steps down a round at a time, and buyers **drop to the price they paid**, coloured by outcome. The current Trail Runner 2 seed-42 run shows **less than a 20% banner, in red,** across the tested floor sweep. Explain that result honestly; do not promise that a slider turns it around, or alter the population to manufacture one. **Adopt** only after reviewing the current rule output. The red-team result is a separate validation artifact.
6. **(2:35)** **PAUSE.** Next message: *"The owner's paused deals — list price stands."* Un-pause.
7. **(2:42) Finale — planned, not built on main.** "Real store data, rules in code — so it travels." In ChatGPT: *"Offer Trailhead $125 for the Trail Runner 2."* The same card renders inline — socks thrown in, **$161 for both** — and a new feed row tagged `ChatGPT` appears in her Console. "In AI shopping chats your merchants are a silent row in a catalog. This one talks."
...
**If beat 7 is not built:** it becomes one sentence — "inside ChatGPT is next" — and beat 5 gets the 15 seconds back.

## 4. HTN rooms — 5 minutes (round 1) and 4 minutes (round 2)

**5 minutes:** same beats. Hand over the keyboard **earlier and for longer** — judges love being given the controls. Let a second judge drag a slider on the price race (Floor · Max off · Rounds · Lowball). End on the checkout tab plus the settled price race, not on a slide.

**4 minutes + 1 min Q&A (round 2):** drop beat 6 (PAUSE) into a single click without commentary and shorten beat 5 to: Play → read the banner figure → drag one slider → Adopt (mention "20 attacks, 0 breaches" in words). Keep the real checkout — and the ChatGPT finale only if it is built.
...
**OpenAI:** show what the understand step — an OpenAI model routed through Backboard — makes of a messy sentence (*"uhh i could maybe do like 115 if socks are in?"*), then the app inside ChatGPT only if it is built, then **read one entry from [`codex-log.md`](codex-log.md)** — the prize text asks for "one concrete way Codex improved your process or outcome". Pick the entry before Sunday.
...
- [ ] **Demo laptop:** store password typed into the demo browser · owner signed in to the Console · tabs in order: storefront TR3 page · Console · ChatGPT (only if that beat is built) · Shopify admin · `codex-log.md` · notifications off · charger · phone hotspot
- [ ] **Safety nets:** pre-minted checkout link with a long expiry · the same server running idle on the laptop behind a tunnel · backup video on the desktop
- [ ] **Rehearsal:** 3 clean runs at gate 2 · 5 more Sunday morning · ~20 phrasings of an offer tried in the storefront chat · each of us can give the 3-minute version alone
...
|---|---|---|
| ChatGPT (beat 7 not built, or it fails) | Do everything on the storefront; skip beat 7 | "Inside ChatGPT is next." |
| The cloud host | Switch to the server already running on the laptop (tunnel URL) | "Same code, running here." |
| Backboard | Engine + template lines, automatically after the timeout (6.5 s by default, `BACKBOARD_TIMEOUT_MS`) | "Backboard's down, so you're seeing the engine with template lines — same prices, same floor." |
| The understand call (through Backboard) | A code parser reads the offer, automatic | Nothing |
| Code won't apply | Open the pre-minted checkout link | "Same price, minted earlier." |
| Supabase | Nothing: policy is cached, deal writes retry | Nothing |
| Wi-Fi | Phone hotspot; the price race runs in the browser once the Console has loaded | Nothing |
...
- **"Isn't this Nibble?"** Haggling widgets exist. None lets the owner rehearse against a population of hagglers before launch. And ours trades — bundles, timing, older stock — by default.
- **"Won't everyone just haggle?"** Nothing is free. Every step down costs a bigger cart, a purchase now, or taking stock she wants gone. New stock doesn't move without a convincing reason, with no reason the early rounds hold at list, and nothing passes her discount cap.
- **"What stops me talking it below cost?"** Nothing you type can reach a price. The model picks from a menu it didn't write, the check throws away any number it invents, and the floor is re-checked from fresh Shopify cost data before a code exists. The menu itself is property-tested: 8 properties × 1,000 seeded cases on the menu every shopper is priced from — `pnpm test:props`. Try.
...
- **"Why did it walk when I offered more than her floor?"** Because how far a price moves depends on stock age, the round and the reason you give — always inside her floor and her discount cap — not on how hard you push. The Gym counts those as *deals missed* — that number is what tells her to lower the floor or wait for the stock to age.
- **"Is personalised pricing fair / legal?"** Negotiated pricing is legal in Canada. Ours isn't personalised: price depends on the cart, its cost, stock age, the round and the reason you state — never on who you are. Memory changes suggestions, not prices. No drip pricing, no fake urgency. Opt-in, disclosed, same rules for everyone.
- **"Is the Gym calibrated?"** No — the label says synthetic, rule-based and seeded, and shows the personas. Real orders would calibrate it. And it's allowed to tell her she's losing: you saw the banner figure in red.
- **"Are those 300 shoppers LLM agents?"** No — rule-based simulated shoppers, which is why the run is instant and reproducible. (If "Gym voices" shipped: "the larger dots are LLM-driven, and labelled.")
...
- **"Why ChatGPT *and* the storefront?"** (Only if beat 7 is built.) Same engine, same public card. It shows the store's agent is independent of the surface.
- **"Does a haggle follow me from the site into ChatGPT?"** (Only if beat 7 is built.) No — one negotiation lives on one surface. Her Console tags each feed row with its surface.
...
- ❌ Vendor-claimed competitor stats. ❌ "It works inside ChatGPT" unless beat 7 is built. ❌ Architecture before the ten-second mark.
...
- [ ] 5 screenshots, storefront first: the storefront haggle · the real Shopify Checkout · the Console feed with a red row · the Approve card · the Gym's price race (+ the card inside ChatGPT only if beat 7 is built)
