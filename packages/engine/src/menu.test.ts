...
it("offers TR2 at $120 and TR2 with gaiters at $144 for $120 on TR3 in round 1", () => {
  ...
  if (result.outcome !== "menu") throw new Error("Expected a counter menu");
  ...
  expect(result.internals.C1).toEqual(expect.objectContaining({ cost: 7800, floor: 9750, target: 11982, profit: 4200 }));
});
...
it("accepts an at-ask offer and rounds a higher partial-dollar offer upward", () => {
  const atAsk = buildMenu({ main: tr3, addOns: [], catalog: [tr3], offer: 16900, round: 1, floorPct: 25, now });
  expect(atAsk).toEqual({ outcome: "accept", total: 16900 });

  const partialDollar = buildMenu({ main: tr3, addOns: [], catalog: [tr3], offer: 17001, round: 3, floorPct: 25, now });
  expect(partialDollar).toEqual({ outcome: "accept", total: 17100 });
});

it.each([1, 2, 3, 4] as const)("does not move a new-stock TR3 shoe in round %i", (round) => {
  const result = buildMenu({ main: tr3, addOns: [], catalog: [tr3], offer: 12000, round, floorPct: 25, now });
  expect(result.outcome).toBe("menu");
  if (result.outcome !== "menu") throw new Error("Expected a counter menu");
  expect(result.options).toEqual(expect.arrayContaining([
...
it("accepts a TR2 round-3 offer above its ask but counters below the ask", () => {
  expect(buildMenu({ main: tr2, addOns: [], catalog: [tr2], offer: 14000, round: 3, floorPct: 25, now }))
    .toEqual({ outcome: "accept", total: 14000 });

  const belowAsk = buildMenu({ main: tr2, addOns: [], catalog: [tr2, ridgeLite], offer: 12500, round: 3, floorPct: 25, now });
  expect(belowAsk.outcome).toBe("menu");
  if (belowAsk.outcome !== "menu") throw new Error("Expected a counter menu");
...
it("labels the held price final in round 4", () => {
  const result = buildMenu({ main: tr2, addOns: [], catalog: [tr2], offer: 11500, round: 4, floorPct: 25, now });
  expect(result.outcome).toBe("menu");
  if (result.outcome !== "menu") throw new Error("Expected a counter menu");
  expect(result.options).toEqual(expect.arrayContaining([
...
it("closes when the requested product has no cost and omits unavailable alternatives", () => {
  expect(buildMenu({ main: { ...tr3, cost: null }, addOns: [], catalog: [tr3], offer: 12000, round: 1, floorPct: 25, now }))
    .toEqual({ outcome: "closed", reason: "missing_cost" });
...
it("only states catalog-backed facts, including unknown stock age", () => {
  const result = buildMenu({ ...input, main: { ...tr3, stockedAt: null }, addOns: [socks], catalog: [tr3, ridgeLite, socks] });
  if (result.outcome !== "menu") throw new Error("Expected menu");
...
it("rounds every displayed amount, including the list total, upward", () => {
  ...
  const result = buildMenu({ ...input, main: product });
  if (result.outcome !== "menu") throw new Error("Expected menu");
...
it("offers no alternatives for $140 on TR2 round 1", () => {
  const result = buildMenu({ ...input, main: tr2, addOns: [socks], catalog: [tr2, ridgeLite, socks], offer: 14000 });
  if (result.outcome !== "menu") throw new Error("Expected menu");
...
it("filters unusable variants and ranks safe options by profit then age", () => {
  ...
  const result = buildMenu({ ...input, addOns: [socks, gaiters, { ...socks, variantId: "no-addon-cost", cost: null }, { ...socks, variantId: "sold-addon", inStock: false }], catalog, size: "10" });
  if (result.outcome !== "menu") throw new Error("Expected menu");
  ...
  for (const [i, option] of result.options.entries()) {
    ...
    expect(option.total).toBeGreaterThan(result.internals[option.id]!.cost);
    expect(option.total).toBeGreaterThanOrEqual(result.internals[option.id]!.floor);
    if (i > 0) expect(result.internals[option.id]!.profit).toBeLessThanOrEqual(result.internals[result.options[i - 1]!.id]!.profit);
  ...
  const tied = buildMenu({ ...input, addOns: [], catalog: [ridgeLite, older], size: "10" });
  if (tied.outcome !== "menu") throw new Error("Expected menu");
