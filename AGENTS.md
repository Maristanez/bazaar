...
- `research/` — read before researching an external API; the answer may already be there.
...
6. **Keys are read from server environment variables.** The browser bundle carries the Supabase anon key and nothing else.
...
- **Engine work:** `tdd` with fast-check properties — invariants 1 and 2 are properties.
...
- **Gym visuals** (dot histogram, swarm, reference lines): `dataviz` for the chart, `impeccable` for the polish pass.
