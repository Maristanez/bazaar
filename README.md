...
The storefront theme is a Shopify CLI theme, outside the pnpm workspace: from `apps/storefront/`, `shopify theme dev --store b8wzw0-h3.myshopify.com`, and `shopify theme check` before pushing. **The agent on the page (Jarvis):** hands-free voice, "Hey Jarvis", page awareness, and the autonomous chores he does on the page (open products, sizes, quantities, the cart, navigation) are one file each under `apps/storefront/assets/juniper-*.js`; [`apps/storefront/README.md`](apps/storefront/README.md) lists every ability and the `window.BazaarChat` seam they plug into. **Changing the store or the shopkeeper chat:** [`apps/storefront/README.md`](apps/storefront/README.md) says which file holds what, what belongs in the server or the engine instead, and how to preview, test, deploy (`shopify theme push`) and verify a theme change. Pushing to `main` does not publish the theme, and pushing the theme does not deploy the server. Deploy is Railway (`railway.json`): `pnpm build`, then `pnpm start`, health check on `/health`.

**The shopkeeper agent is not deployed from this repo.** Railway runs the code that calls it. The agent itself lives on **Backboard**: the base assistant (`BACKBOARD_ASSISTANT_ID`) is created by hand in the Backboard dashboard, the three store documents in `infra/seed/` are uploaded to it there and must reach `indexed` before the shopkeeper can answer a sizing or returns question, and each shopper gets a clone of that assistant created at runtime, holding their memory. A deploy ships a client, a key and an assistant id — it creates no assistant and uploads no document. Those dashboard steps outlive every deploy, and a fresh Backboard account or a replaced assistant costs them again. Details: [`docs/SPEC.md`](docs/SPEC.md) §8 and [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) §9.1.
